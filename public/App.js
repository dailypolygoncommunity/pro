...
function App() {
  ...
  const handleConnectWallet = async () => {
    try {
      ...
      handleGetTotalDeposits(); // Fetch total deposits on wallet connect
    } catch (error) {
  ...
  const handleDisconnect = () => {
    ...
    setPreviousReferrer('');
    setReferrer('');
    setTotalDeposits(0); // Reset total deposits on disconnect
  ...
  const handleGetTotalDeposits = async () => {
    if (!account || !contract) {
      console.error("Wallet not connected or contract not initialized");
      return;
    }
    try {
      const totalDepositCount = await contract.methods.getUserTotalDeposits(account).call();
      console.log("Total Deposit Count fetched:", totalDepositCount); // Debug log
      setTotalDeposits(Web3.utils.fromWei(totalDepositCount, 'ether')); // Convert to ether if necessary
    } catch (error) {
      console.error("Failed to retrieve total deposits:", error);
    }
  };
  ...
  const handleMakeInvestment = async (event) => {
    ...
    try {
      ...
      handleGetTotalDeposits(); // Fetch total deposits after investment
    } catch (error) {
        ...
        <div className="container">
          {account ? (
            <div className="dashboard">
              ...
              <p className="total-deposits">
                Your Total Donations: {totalDeposits} POL
              </p>
              <div className="button-group">
                <button onClick={handleGetBonus} className="bonus-button">Check Your Available Grant</button>
                <button onClick={handleGetTotalDeposits} className="bonus-button">Check Your Total Deposits</button>
              </div>
